...
const EMOTION_CODE = {
  0: { label: 'Positive',  type: 'positive', color: '#0daa6b', icon: 'bi-emoji-smile'  },
  1: { label: 'Negative',  type: 'negative', color: '#e53e3e', icon: 'bi-emoji-frown'  },
  2: { label: 'Normal',    type: 'neutral',  color: '#d08c00', icon: 'bi-emoji-neutral' },
};
...
async function checkServerStatus() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      setServerStatus('online', data.model || 'พร้อมใช้งาน');
    } else {
...
function setServerStatus(state, msg) {
  ...
  const styles = {
    online:   { bg: 'rgba(13,170,107,0.15)', color: '#0daa6b', border: 'rgba(13,170,107,0.3)', text: 'เชื่อมต่อแล้ว' },
    ...
    unknown:  { bg: 'rgba(255,165,0,0.12)',  color: '#d08c00', border: 'rgba(208,140,0,0.3)',  text: 'ไม่ทราบสถานะ' },
  };
...
async function requestCameraPermission() {
  ...
  try {
    ...
    const msgs = { NotAllowedError: 'ถูกปฏิเสธสิทธิ์ กรุณาอนุญาตกล้องในแถบที่อยู่เบราว์เซอร์ แล้วรีโหลดหน้า', NotFoundError: 'ไม่พบกล้องในอุปกรณ์นี้ กรุณาเชื่อมต่อ Webcam แล้วลองใหม่' };
...
function refreshNavProfile(prof) {
  ...
  document.getElementById('dd-id').textContent = 'ID: ' + currentUser + (prof.uid ? ' | Unique ID: ' + prof.uid : '');
  const letter = dn.charAt(0).toUpperCase() || '?';
  document.getElementById('nav-avatar-letter').textContent = letter;
  const navImg = document.getElementById('nav-avatar-img');
  if (prof.avatar) { navImg.src = prof.avatar; navImg.style.display = 'block'; document.getElementById('nav-avatar-letter').style.display = 'none'; }
  else { navImg.style.display = 'none'; document.getElementById('nav-avatar-letter').style.display = 'block'; }
}
  ...
  if (savedToken) {
    DataService.loadProfile().then(prof => {
      ...
      if (cached && cached.username) {
        ...
        loginAs(cached.username, cached);
      }
    });
  }
});
...
// ===== DROPDOWN =====
function toggleDropdown() { document.getElementById('profile-btn').classList.toggle('open'); document.getElementById('profile-dropdown').classList.toggle('open'); }
function closeDropdown() { document.getElementById('profile-btn').classList.remove('open'); document.getElementById('profile-dropdown').classList.remove('open'); }
...
async function saveAccountSettings(event) {
  ...
  try {
    ...
    refreshNavProfile();
    addLog('บันทึกข้อมูลบัญชี' + (dn ? ' ชื่อ: ' + dn : '') + (uid ? ' Unique ID: ' + uid : ''));
    btn.innerHTML = '<i class="bi bi-check-lg"></i>บันทึกแล้ว!'; btn.style.background = 'var(--positive)';
    setTimeout(() => { btn.innerHTML = orig; btn.style.background = ''; }, 1500);
  } catch (e) {
    btn.innerHTML = '<i class="bi bi-x-lg"></i>ผิดพลาด'; btn.style.background = 'var(--negative)';
    setTimeout(() => { btn.innerHTML = orig; btn.style.background = ''; }, 1500);
  }
}
...
function toggleDark(el) {
  ...
  const fg = isDark ? '#8b97c0' : '#5a6480';
  Chart.defaults.color = fg;
  Chart.defaults.borderColor = isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)';
  if (pieChart) [pieChart, barChart, lineChart].forEach(c => { c.options.plugins.legend.labels.color = fg; c.update(); });
}
...
// =================================================================
//  📸 CAPTURE FRAME → ส่ง Base64 → FastAPI Server → รับ emotion_code
// =================================================================
async function captureAndSend() {
  ...
  try {
    ...
    // ตัวอย่าง response: { "status": "success", "emotion_code": 0, "faces": 3, "positive": 2, "negative": 1, "neutral": 0 }
...
function handleServerResponse(data) {
  ...
  // จำนวนใบหน้า — server อาจส่งมาด้วย หรือไม่ส่งก็ได้
  ...
  // ── อัปเดต face count ──
  document.getElementById('face-count').textContent = total;
  ...
  // ── Log ──
  addLog(`✅ รอบ ${S.rounds}: ${total} คน | emotion_code=${emoCode} (${emo.label}) | +${pos} -${neg} ~${neu}`);
  ...
  DataService.saveScan({
    ...
    faces: total, positive: pos, negative: neg, neutral: neu,
    emotionCode: emoCode
...
function addLog(msg) {
  ...
  if (a.children.length > 30) a.removeChild(a.lastChild);
}
...
function initCharts() {
  ...
  pieChart  = new Chart(document.getElementById('pieChart'),  { type: 'doughnut', data: { labels: ['Positive', 'Negative', 'Normal'], datasets: [{ data: [0, 0, 0], backgroundColor: ['#0daa6b', '#e53e3e', '#d08c00'], borderWidth: 0, hoverOffset: 4 }] }, options: { responsive: true, plugins: { legend: { position: 'bottom', labels: { color: '#5a6480', padding: 14, font: { size: 12 } } } } } });
  barChart  = new Chart(document.getElementById('barChart'),  { type: 'bar',      data: { labels: [], datasets: [{ label: 'Positive', data: [], backgroundColor: 'rgba(13,170,107,0.8)', borderRadius: 4 }, { label: 'Negative', data: [], backgroundColor: 'rgba(229,62,62,0.8)', borderRadius: 4 }, { label: 'Normal', data: [], backgroundColor: 'rgba(208,140,0,0.8)', borderRadius: 4 }] }, options: { responsive: true, plugins: { legend: { labels: { color: '#5a6480', font: { size: 12 } } } }, scales: { x: { stacked: true, grid: { display: false }, ticks: { color: '#8b94b3' } }, y: { stacked: true, beginAtZero: true, ticks: { color: '#8b94b3' } } } } });
  lineChart = new Chart(document.getElementById('lineChart'), { type: 'line',     data: { labels: [], datasets: [{ label: 'Positive', data: [], borderColor: '#0daa6b', backgroundColor: 'rgba(13,170,107,0.08)', tension: 0.4, fill: true, pointRadius: 3 }, { label: 'Negative', data: [], borderColor: '#e53e3e', backgroundColor: 'rgba(229,62,62,0.08)', tension: 0.4, fill: true, pointRadius: 3 }] }, options: { responsive: true, plugins: { legend: { labels: { color: '#5a6480', font: { size: 12 } } } }, scales: { x: { ticks: { color: '#8b94b3' } }, y: { beginAtZero: true, ticks: { color: '#8b94b3' } } } } });
}
...
function updateDashboard() {
  ...
  barChart.data.datasets[0].data = r.map(x => x.pos); barChart.data.datasets[1].data = r.map(x => x.neg); barChart.data.datasets[2].data = r.map(x => x.neu); barChart.update();
  ...
  lineChart.data.datasets[0].data = r.map(x => x.pos); lineChart.data.datasets[1].data = r.map(x => x.neg); lineChart.update();
}
...
function exportReport() {
  ...
  const u = URL.createObjectURL(b); const a = document.createElement('a'); a.href = u; a.download = 'STU-Check-Report.csv'; a.click(); URL.revokeObjectURL(u);
